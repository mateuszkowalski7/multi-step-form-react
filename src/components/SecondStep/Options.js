...
const Options = () => {
  ...
  const ArcadeOptionHandler = () => {
    setOption("arcade");
    dispatch(
      buttonActions.addItems({
        type: !isChecked ? "MO" : "YR",
        programName: "Arcade",
        programPrice: !isChecked ? 9 : 90,
      })
    );
  };

  const AdvencedOptionHandler = () => {
    setOption("advenced");
    dispatch(
      buttonActions.addItems({
        type: !isChecked ? "MO" : "YR",
        programName: "Advenced",
        programPrice: !isChecked ? 12 : 120,
      })
    );
  };

  const ProOptionHandler = () => {
    setOption("pro");
    dispatch(
      buttonActions.addItems({
        type: !isChecked ? "MO" : "YR",
        programName: "Pro",
        programPrice: !isChecked ? 15 : 150,
      })
    );
  };
  ...
  let discount = isChecked && (
    <p style={{ fontSize: "14px", color: "blue" }}>2 months free</p>
  );
      ...
      <div className={classes.SelectPlan}>
        ...
        >
          ...
          <div className={classes.ButtonText}>
            <span>Arcade</span>
            <p>{!isChecked ? "$9/mo" : "$90/yr"}</p>
            {discount}
          </div>
        </button>
        <button
          onClick={AdvencedOptionHandler}
          style={
            option === "advenced"
              ? {
                  backgroundColor: "hsl(217, 100%, 97%)",
        ...
        >
          ...
          <div className={classes.ButtonText}>
            <span>Advenced</span>
            <p>{!isChecked ? "$12/mo" : "$120/yr"}</p>
            {discount}
          </div>
        </button>
        <button
          onClick={ProOptionHandler}
          style={
            option === "pro"
              ? {
                  backgroundColor: "hsl(217, 100%, 97%)",
        ...
        >
          ...
          <div className={classes.ButtonText}>
            <span>PRO</span>
            <p>{!isChecked ? "$15/mo" : "$150/yr"}</p>
            {discount}
          </div>
        </button>
      </div>
      <div className={classes.OptionsAction}>
        <span className={classes.OptionsSpan}>Monthly</span>
        <div className={classes.ToggleBtn}>
          <label className={classes.Switch}>
            <input type="checkbox" onChange={inputCheckedHandler} /> <div></div>
          </label>
        </div>
        <span className={classes.OptionsSpan}>Yearly</span>
      </div>
    </div>
  );
};
